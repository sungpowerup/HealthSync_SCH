...
public class UserServiceAdapter implements UserServicePort {
    ...
    public void validateUserExists(String userId) {
        try {
            log.info("User Service 사용자 존재 확인: userId={}", userId);
            
            // 실제 구현에서는 User Service API 호출하여 존재 여부 확인
            // Mock 구현 - 항상 존재한다고 가정
            
        } catch (Exception e) {
            log.error("User Service 사용자 검증 실패: userId={}, error={}", userId, e.getMessage(), e);
            throw new ExternalApiException("사용자 검증에 실패했습니다.");
        }
    }
}
